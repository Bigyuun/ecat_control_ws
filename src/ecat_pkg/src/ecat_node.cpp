...
int EthercatNode::MapDefaultPdos()
{
    ...
    **/
    ...
    ec_pdo_entry_info_t maxon_epos_pdo_entries[] = {
        ...
        // {OD_ANALOG_OUTPUT_PROPERTIES_1, 16},
        // {OD_ANALOG_OUTPUT_PROPERTIES_2, 16}
    };
    ...
    for(int i = 0; i < g_kNumberOfServoDrivers ; i++){
        ...
        // this->slaves_[i].offset_.analog_output_1    = ecrt_slave_config_reg_pdo_entry(this->slaves_[i].slave_config_,
        //                                                                           OD_ANALOG_OUTPUT_PROPERTIES_1,g_master_domain,NULL);
        // this->slaves_[i].offset_.analog_output_2    = ecrt_slave_config_reg_pdo_entry(this->slaves_[i].slave_config_,
        //                                                                           OD_ANALOG_OUTPUT_PROPERTIES_2,g_master_domain,NULL);
...
int16_t EthercatNode::ReadAnalogOutput1ViaSDO(int index)
{
    SDO_data pack ; 
    int16_t analog_output_1;  
    pack.slave_position = index;
    pack.index = OD_ANALOG_OUTPUT_PROPERTIES_1 ; 
    pack.sub_index = 0 ;
    pack.data_sz = sizeof(int16_t);
    if(SdoRead(pack)){
       std::cout << "Error while reading Analog Output #1 " << std::endl;
        return -1; 
    }
    analog_output_1 = (int16_t)(pack.data);
    return analog_output_1 ; 
}
int16_t EthercatNode::ReadAnalogOutput2ViaSDO(int index)
{
    SDO_data pack ; 
    int16_t analog_output_2;  
    pack.slave_position = index;
    pack.index = OD_ANALOG_OUTPUT_PROPERTIES_2 ; 
    pack.sub_index = 0 ;
    pack.data_sz = sizeof(int16_t);
    if(SdoRead(pack)){
       std::cout << "Error while reading Analog Output #2 " << std::endl;
        return -1; 
    }
    analog_output_2 = (int16_t)(pack.data);
    return analog_output_2 ; 
}
